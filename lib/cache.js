...
var url = require('url'), factory = require('./drivers/factory'), LruCache = require('./lru');

var legacyConfigSupport = function(config, log) {
    if (config.redisHost || config.redisPort) {
        log('WARNING: old redis syntax is deprecated! Please migrate to ' + '{"driver": ["redis://host:port", "redis://masterhost:port"]}');
    }
    var driver = 'redis://';
...
function Cache(config, handlers) {
    ...
    var logHandler = handlers.logHandler || console.log, debugHandler = handlers.debugHandler || console.log;
    this.config = config;
...
Cache.prototype.markDeadBackend = function(backendInfo) {
    this.client.mark(backendInfo.frontend, backendInfo.backendId, backendInfo.backendUrl, backendInfo.backendLen, this.config.server.deadBackendTTL, function() {
    });
...
Cache.prototype.getBackendFromHostHeader = function(host, callback) {
    ...
    readFromCache(host, function(rows) {
        ...
        var metrics_session = backends[1];
        var log_session = backends[2];
        var SNICertificate = backends[3];
        var SNIKey = backends[4];
        var SNIPassphrase = backends[5];

        backends = backends.slice(6);
        // copy to not modify the lru in place
        ...
        var backend = url.parse(backends[index].split('-')[0]);
        backend.id = index;
        // Store the backend index
        backend.frontend = host;
        // Store the associated frontend
        backend.virtualHost = virtualHost;
        // Store the associated vhost
        backend.name = backends[index].split('-')[1] || 'noname';
        backend.len = backends.length;
        if (!backend.hostname) {
            return callback('backend is invalid', 502);
        }
        backend.port = backend.port ? parseInt(backend.port, 10) : 80;
        backend.metrics_session = metrics_session;
        backend.log_session = log_session;
        backend.SNICertificate = SNICertificate;
        backend.SNIKey = SNIKey;
        backend.SNIPassphrase = SNIPassphrase;
 ...
 * error.
 */
Cache.prototype.getSNIFromHostHeader = function(host, callback) {
    host = 'tls:' + host;
    ...
    var readFromCache = function(hostKey, cb) {
        ...
        // The entry is not in the LRU cache, let's do a request on Redis
        this.client.sni(hostKey, function(err, rows) {
            this.lru.set(hostKey, rows);
            cb(rows);
        }.bind(this));
    }.bind(this);
