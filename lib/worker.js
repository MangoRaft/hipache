...
var balancerVersion = require(path.join(__dirname, '..', 'package.json')).version;
...
Worker.prototype.runServer = function (config) {
    ...
    var proxyErrorHandler = function (err, req, res) {
        if (err.code === 'ECONNREFUSED' ||
            ...
            log(req.headers.host + ': backend #' + req.meta?req.meta.backendId:'null' + ' reported an error (' +
    ...
    var staticDir = config.staticDir || [ rootDir, 'static' ].join('/');
    ...
    var errorMessage = function (res, message, code) {
        ...
        var staticPath = function (name) {
             return [ staticDir, '/', 'error_default.html' ].join('');
        };
            ...
            fs.readFile(filePath,function(err, data){
              if (err) throw err;
                ...
                if (res.debug === true) {
                    ...
                    headers['x-debug-version'] = balancerVersion;
                }
                res.writeHead(code, headers);
                res.end(data.toString().replace(new RegExp('{{MESSAGE}}', 'g'),message||'Application Not Responding').replace('{{CODE}}',code));
	...
	var statsPoll = function(req){
        ...
        req.connection.stats.timmer = setInterval(function(){
			...
			self.metrics.send({
				...
				session: req.meta.metrics_session,
        ...
        (function () {
            ...
            res.end = function () {
                ...
                self.accessLog.log({
                    ...
                    method: req.method,
                    url: req.url,
                    name: req.meta.backendname,
                    httpVersion: req.httpVersion,
                    statusCode: res.statusCode,
                    socketBytesWritten: socketBytesWritten,
                    referer: req.headers.referer,
                    userAgent: req.headers['user-agent'],
                    backendUrl: req.meta.frontend,
                    session: req.meta.log_session
                });
            ...
            this.cache.getBackendFromHostHeader(req.headers.host, function (err, code, backend) {
                ...
                if (err) {
                	req.meta = {
	                    ...
	                    virtualHost: req.headers.host,
	                    backendUrl: req.headers.host,
	                    backendname:'NOTFOUND',
	                    metrics_session: self.config.metrics.session,
	                    log_session: self.config.logging.session
	                };
                	...
                	req.meta = {
	                    ...
	                    metrics_session: backend.metrics_session,
	                    log_session: backend.log_session
	                };
                }
    ...
    (function () {
       ...
       httpsServer =  https.createServer({
            ...
            SNICallback: function(servername, cb) {
                self.cache.getSNIFromHostHeader(servername, function(err, data) {
                    ...
                    if(data){
                        ...
                        var ctx = tls.createSecureContext(sniOptions);
                      return cb(null, ctx);
                    }
                    log('cert fail for host: '+servername);
